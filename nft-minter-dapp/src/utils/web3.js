...
export const approveUSDC = async (address, amount) => {
  ...
  const approveWalletConnect = async () => {
    const transactionParameters = {
      to: usdcContractAddress,
      from: address,
      'data': usdcContract.methods.approve(contractAddress, approvalAmount).encodeABI()
    };
    try {
...
export const mintNFT = async(walletAddress, paymentType) => {
  ...
  const metadata = {
    ...
    description: 'The Stackup Founding Membership NFT offers special benefits for early users',
  }
  ...
  const sendTransactionMetaMask = async () => {
    ...
    try {
      ...
      return {
        success: true,
        status:
          `✅ Check out your transaction on Polygonscan: ${process.env.REACT_APP_POLYGON_TX_HASH_PATH}${txHash}`
      };
    } catch (error) {
      return {
        success: false,
  ...
  const sendTransactionWalletConnect = async () => {
    ...
    const transactionParameters = {
      to: contractAddress,
      from: walletAddress,
      'data': nftContract.methods.mintToAddress(walletAddress, tokenURI, paymentInUSDC).encodeABI()
    };
    try {
      const txHash = await web3.eth.sendTransaction(transactionParameters);
      return {
